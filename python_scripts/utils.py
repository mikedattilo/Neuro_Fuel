...
def save_audio_bytes(audio_bytes, filetype="mp3", file_dir=None):
    try:
        from pathlib import Path
        file_dir = Path(file_dir)
    except TypeError:
        raise ValueError("file_dir must be a valid Path object or a string representing a directory path.")
    file_path = file_dir / f"audio.{filetype}"
